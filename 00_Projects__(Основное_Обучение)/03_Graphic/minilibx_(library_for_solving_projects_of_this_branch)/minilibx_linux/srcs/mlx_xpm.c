...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/12/26 19:56:12 by wescande          #+#    #+#             */
/*   Updated: 2016/12/26 20:08:45 by wescande         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

/*
** Mise a la norme pqar wescande
...
void	*mlx_int_parse_xpm(t_xvar *xvar,
							void *info, int info_size, char *(*f)())
{
	int			pos;
	char		*line;
	char		**tab;
	...
	while (i--)
	{
		...
		if (method)
			colors_direct[mlx_int_get_col_name(line, cpp)] =
				rgb_col >= 0 ? mlx_int_get_good_color(xvar, rgb_col) : rgb_col;
		else
		{
			colors[i].name = mlx_int_get_col_name(line, cpp);
			colors[i].col = rgb_col >= 0 ?
						mlx_int_get_good_color(xvar, rgb_col) : rgb_col;
		}
		free(tab);
	}
								...
								int *width, int *height)
{
	...
	if (fd == -1 || (size = lseek(fd, 0, SEEK_END)) == -1 ||
